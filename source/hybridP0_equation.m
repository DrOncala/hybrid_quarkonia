function [Energy,Y_norm,x] = hybridP0_equation(mass,J,index_n,Vname)
%disp('**************************************************')
%disp('Hybrid decoupled P^0:')
%disp('**************************************************')
format long;
...
function pM=potentialMatrix(x) 
  global global_J;
  global global_mass;
  
  for i=1:4
  pM(1,1,i)=potentialTerm1(x(i),global_J,global_mass);
  end
end

#### DEFINE THE POTENTIL TERMS FOR THE MATRIX
function pT1=potentialTerm1(x,J,mass) 
  global global_Vname;
  
  pT1=(J*(J+1))/(x.^2)+mass*potential(global_Vname,x,mass);
end 
