% --------------------------------------------------
% Hybrid Sigma_u^- to Quarkonium Sigma_g^+ decay overlap
    ...
    for n_H = 1:3
        ...
        for L = 0:2
            for n_Q = 1:2
                ...
                % Interpolate wavefunctions on a common grid
                ...
                Int_QQ  = trapz(new_x, new_Y_Q .* new_Y_Q);
                ...
                results = [results; mass, J, n_H, L, n_Q, ...
                           ...
                           Int_HxQ, Int_HQ, Int_QxQ, Int_QQ, C];
            end
        end
    end
end
...
% -------------------------------
filename = 'output/hybridPplus0_decay.dat';
...
fprintf(fileID, '#   2  J           (hybrid angular momentum, fixed = 0)\n');
...
fprintf(fileID, '#   9  Int_HxQ     (overlap integral with r factor)\n');
fprintf(fileID, '#  10  Int_HQ      (wavefunction overlap)\n');
fprintf(fileID, '#  11  Int_QxQ     (<r> expectation for quarkonium)\n');
fprintf(fileID, '#  12  Int_QQ      (quarkonium normalization test)\n');
fprintf(fileID, '#  13  C           (3*(0.215*<r>_Q - 0.028)/DeltaE)\n');
fprintf(fileID, '# -------------------------------------------------------------------------\n');

% Write numerical results
for i = 1:size(results, 1)
    fprintf(fileID, ...
        '%8.4f  %3d  %3d  %3d  %3d  %15.8f  %15.8f  %15.8f  %15.8e  %15.8e  %15.8e  %15.8e  %15.8e\n', ...
        results(i,1), results(i,2), results(i,3), results(i,4), results(i,5), ...
        results(i,6), results(i,7), results(i,8), results(i,9), ...
        results(i,10), results(i,11), results(i,12), results(i,13));
end
